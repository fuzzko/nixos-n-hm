...
task.switch-hm() {
  nix run nixpkgs#nh -- home switch -c komo -b backup . -- --impure "$@" --show-trace
...
task.switch-nixos() {
  ...
  nix run nixpkgs#nh -- os switch -H "$config" . -- --impure "$@" --show-trace
}
...
task.boot-nixos() {
  ...
  nix run nixpkgs#nh -- os boot -H "$config" . -- --impure "$@" --show-trace
}
